...
impl<'a, W: Write> Serializer<'a, W> {
    ...
    fn serialize_sys(&mut self, sys: &TransitionSystem) -> std::io::Result<()> {
        ...
        for (e, id_of_target) in alias_targets {
            ...
            writeln!(
                self.writer,
                "{line_id} uext {sort} {id_of_target} 0 {name}"
            )?;
        }
...
fn compute_alias_needed(
    ...
    let mut last_label: rustc_hash::FxHashMap<ExprRef, String> =
...
fn write_node<W: Write>(
    ...
    match expr {
        ...
        Expr::BVImplies(_, _) => {
            write!(writer, "{id} implies {sort} {} {}", children[0], children[1])?
        }
        Expr::BVGreater(_, _) => {
            write!(writer, "{id} ugt {sort} {} {}", children[0], children[1])?
        }
        ...
        Expr::BVAnd(_, _, _) => {
            write!(writer, "{id} and {sort} {} {}", children[0], children[1])?
        }
        Expr::BVOr(_, _, _) => {
            write!(writer, "{id} or {sort} {} {}", children[0], children[1])?
        }
        Expr::BVXor(_, _, _) => {
            write!(writer, "{id} xor {sort} {} {}", children[0], children[1])?
        }
        ...
        Expr::BVAdd(_, _, _) => {
            write!(writer, "{id} add {sort} {} {}", children[0], children[1])?
        }
        Expr::BVMul(_, _, _) => {
            write!(writer, "{id} mul {sort} {} {}", children[0], children[1])?
        }
        ...
        Expr::BVSub(_, _, _) => {
            write!(writer, "{id} sub {sort} {} {}", children[0], children[1])?
        }
